...
def test(model, device, train_test, test_loader, loud, loss_func, num_classes=10):
    test_loss = 0
    correct = 0
    model.eval()
    with torch.inference_mode():
        for data, target in test_loader:
            ...
            onehots = torch.nn.functional.one_hot(target, num_classes=num_classes).to(
                device
            )
...
def update_metrics(
    ...
    loss, acc = test(
        model, device, train_test, loader, loud, loss_func, num_classes=num_classes
    )
